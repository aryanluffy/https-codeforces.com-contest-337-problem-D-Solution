...
int sp[100001];
v32 Tree[100001];
vt <pair<int,int>> max1(100001),max2(100001);
v32 pmax(100001);
void find_max(int u,int p){
   for(int i=0;i<Tree[u].size();i++){
     ...
     if(x==p)continue;
     find_max(x,u);
     if(max1[u].first<max1[x].first+1){
       max1[u].first=max1[x].first+1;
       max1[u].second=x;
     }
   }
   for(int i=0;i<Tree[u].size();i++){
     ...
     if(x==p)continue;
     if(x!=max1[u].second){
        if(max2[u].first<max1[x].first+1){
          max2[u].first=max1[x].first+1;
          max2[u].second=x;
        }
     }
}
}
void find_pmax(int u,int p){
   for(int i=0;i<Tree[u].size();i++){
     int x=Tree[u][i];
     if(x==p)continue;
     if(x!=max1[u].second){
         pmax[x]=max(1+pmax[u],1+max1[u].first);
     }
     else{
       pmax[x]=max(1+pmax[u],1+max2[u].first);
     }
     find_pmax(x,u);
   }
}
...
{ 
  ...
  f(i,1,n+1){
    if(!sp[i]){max1[i].first=max2[i].first=-MAXN;}
  }
  f(i,0,n-1){
    int u,v;cin>>u>>v;
    Tree[u].pb(v);
